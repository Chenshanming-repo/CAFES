...
class CAFES_Bottleneck(nn.Module):
    def __init__(self, n_conv_neurons, n_fc_neurons=2048, depth=43, n_classes=2, shortcut=False):
        super(CAFES_Bottleneck, self).__init__()

        layers = []
        fc_layers = []

        layers.append(nn.Conv1d(5, n_conv_neurons[0], kernel_size=19, padding=5, stride=3))
        layers.append(nn.BatchNorm1d(n_conv_neurons[0]))
        layers.append(nn.ReLU())
        layers.append(nn.MaxPool1d(2, padding=1, stride=2))
        ...
        layers.append(BottleneckResBlock(input_dim=n_conv_neurons[1], n_filters=n_conv_neurons[1], kernel_size=3,
                                         shortcut=shortcut))
        for _ in range(n_conv_block_1 - 1):
            layers.append(BottleneckResBlock(input_dim=n_conv_neurons[1], n_filters=n_conv_neurons[1], kernel_size=3,
                                             shortcut=shortcut))
        ...
        ds = nn.Sequential(nn.Conv1d(n_conv_neurons[1], n_conv_neurons[2], kernel_size=1, stride=1, bias=False),
                           nn.BatchNorm1d(n_conv_neurons[2]))
        layers.append(BottleneckResBlock(input_dim=n_conv_neurons[1], n_filters=n_conv_neurons[2], kernel_size=3,
                                         shortcut=shortcut, downsample=ds))
        for _ in range(n_conv_block_2 - 1):
            layers.append(BottleneckResBlock(input_dim=n_conv_neurons[2], n_filters=n_conv_neurons[2], kernel_size=3,
                                             shortcut=shortcut))
        layers.append(nn.MaxPool1d(kernel_size=3, stride=2, padding=1))  # l = length / 4

        ds = nn.Sequential(nn.Conv1d(n_conv_neurons[2], n_conv_neurons[3], kernel_size=1, stride=1, bias=False),
                           nn.BatchNorm1d(n_conv_neurons[3]))
        layers.append(BottleneckResBlock(input_dim=n_conv_neurons[2], n_filters=n_conv_neurons[3], kernel_size=3,
                                         shortcut=shortcut, downsample=ds))
        for _ in range(n_conv_block_3 - 1):
            layers.append(BottleneckResBlock(input_dim=n_conv_neurons[3], n_filters=n_conv_neurons[3], kernel_size=3,
                                             shortcut=shortcut))
        layers.append(nn.MaxPool1d(kernel_size=3, stride=2, padding=1))  # l = length / 8

        ds = nn.Sequential(nn.Conv1d(n_conv_neurons[3], n_conv_neurons[4], kernel_size=1, stride=1, bias=False),
                           nn.BatchNorm1d(n_conv_neurons[4]))
        layers.append(BottleneckResBlock(input_dim=n_conv_neurons[3], n_filters=n_conv_neurons[4], kernel_size=3,
                                         shortcut=shortcut, downsample=ds))
        for _ in range(n_conv_block_4 - 1):
            layers.append(BottleneckResBlock(input_dim=n_conv_neurons[4], n_filters=n_conv_neurons[4], kernel_size=3,
                                             shortcut=shortcut))
