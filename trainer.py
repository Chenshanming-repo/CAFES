...
def train(model, pos_train_generator, neg_train_generator, pos_valid_generator, neg_valid_generator, log, device):
	...
	scheduler = torch.optim.lr_scheduler.LambdaLR(
			optimizer, lr_lambda=lambda epoch: 1 / (2**epoch), last_epoch=-1)
...
if __name__ == '__main__':
    ...
    #valid_process_method = valid_non_normalization if not args.norm else valid_normalization
    #train_process_method = train_non_normalization if not args.norm else train_normalization
    ...
    #print("[DEBUG]Torch tensor bytes of pos_train_data:", pos_train_data.nbytes)
    #print("[DEBUG]Torch tensor bytes of neg_train_data:", neg_train_data.nbytes)
    #memory_report()
    #exit(-1)
    ...
    # test_method = test_non_normalization_add_features if not args.norm else test_normalization_add_features
    # test_method = test_non_normalization_add_features
    test_method = test
    model.load_state_dict(torch.load(os.path.join(args.output, "model.pth")))
